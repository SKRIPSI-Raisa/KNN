...
def extract_rgb_features(image_rgb):
    """
    Mengekstrak nilai rata-rata (mean) dan standar deviasi dari channel R, G, B.
    Sesuai dengan implementasi pada notebook pelatihan dan camera.py.
    """
    features = []
    for c in range(3):
        channel = image_rgb[:, :, c]
...
def extract_glcm_features(gray_image):
    """
    Mengekstrak fitur tekstur GLCM (Kontras, Energi, Homogenitas, Korelasi).
    Sesuai dengan implementasi pada notebook pelatihan dan camera.py.
    """
    glcm = graycomatrix(
        gray_image,
...
async def lifespan(app: FastAPI):
    ...
    # Dapatkan path absolut relatif terhadap app.py
    ...
    # Bersihkan sumber daya (jika ada)
...
# Aktifkan CORS untuk integrasi frontend
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def get_metrics():
    """
    Menyediakan metrik asli model KNN dari tahap pelatihan.
    Sesuai dengan antarmuka ModelMetrics yang diharapkan pada aplikasi Next.js.
    """
    return {
        "accuracy": 0.8361,
...
async def classify_image(file: UploadFile = File(...)):
    ...
    try:
        # 1. Baca byte dari gambar yang diunggah
        ...
        # 2. Pra-pemrosesan: Ubah ukuran gambar agar sesuai dengan ukuran saat pelatihan
        ...
        # 6. Tingkat Keyakinan (Confidence Score) / Probabilitas
        try:
            probabilities = model.predict_proba(features_scaled)[0]
            confidence = float(probabilities[pred_class_idx] * 100)
        except Exception:
            confidence = 100.0  # Alternatif jika predict_proba tidak didukung atau gagal
            
        # 7. Dapatkan Detail Tetangga Terdekat (Jarak & Label)
        neighbors = []
        try:
            ...
            for rank_idx, (idx, dist) in enumerate(zip(indices, distances)):
                # Rekonstruksi label kelas tetangga menggunakan model._y
        ...
        # 8. Encode gambar yang diproses (128x128) ke base64 untuk pratinjau
        _, buffer = cv2.imencode('.jpg', img_resized)
        img_base64 = f"data:image/jpeg;base64,{base64.b64encode(buffer).decode('utf-8')}"
        
        # Tampilkan ringkasan klasifikasi di terminal
        print("\n" + "=" * 50)
...
def pred_label_mapping(label: str) -> str:
    """
    Menstandarkan label menjadi format yang mudah dibaca.
    Contoh: "organik" -> "Organik", "non_organik" -> "Non-Organik"
    """
